...
get_active_directory() {
    ...
    dc_os=$(
        tshark -r $PCAP -Y "smb&&ip.addr==$dc_ip" -T fields -e smb.native_os | \
        grep -v '^\s*$' | \
        sort | \
        uniq
    )
...
get_win_computers() {
    ...
    local win_hosts=$(
        tshark -r $PCAP -Y kerberos -T fields -e eth.src -e ip.src -e kerberos.CNameString | \
        grep "\$$" | \
        grep -v $dc_ip | \
        awk '{print toupper($0)}' | \
        sed 's/\$//g' | \
...
get_win_users() {
    ...
    local win_users=$(
        tshark -r $PCAP -Y kerberos.CNameString -T fields -e eth.src -e ip.src -e kerberos.CNameString | \
        grep -v "\$$" | \
        grep -v $dc_ip | \
        column -t | \
        sort | \
